...
def quick_reference(local_path, image_name, image_type, image_os, image_platform):
    ...
    text_holder += (
        "  Intel&reg; " + platform_subs[image_platform] + ", " + os_subs[image_os]
    )
    text_holder += "\n\n"
...
def parse_inherited_file(inherited_file, image_os):
    ...
    parsed_inherited_file = inherited_file.replace(
        "OS_NAME`'patsubst(OS_VERSION,\\.)", image_os1
    )
    ...
    with open(local_path + "/Dockerfile.m4", "r") as fp:
        ...
        while line:
            if "ubuntu" in image_os and not os_flag:
                ...
                entry_holder += inheritance_populate(
                    handler_list, path_subs[inherited_file_1]
                )
...
def create_readme(path, path_components):
    ...
    if image_platform == "QAT":
        my_file.write(
            "Optimized for NGINX web server with compute-intensive operations acceleration with Intel® QuickAssist Technology (Intel® QAT).The docker image can be used in the FROM field of a downstream Dockerfile."
        )
    elif image_name == "dev":
        my_file.write(
            "This is development image aim towards enabling C++ application compilation, debugging (with the debugging, profiling tools) and optimization (with the optimization tools.) You can compile C++ applications with this image and then copy the applications to the corresponding deployment image. "
        )
        if image_type == "analytics":
            my_file.write(
                "Included what are in FFmpeg & GStreamer media analytics images. "
            )
        if image_type == "media":
            my_file.write(
                "Included what are in FFmpeg or GStreamer media creation and delivery images . "
            )
        if image_platform == "SG1":
            my_file.write(
                "Also included Intel hardware accelaration software stack such as media SDK, media driver, gmmlib and libva. "
            )
        ...
        my_file.write(
            "The docker image can be used in the FROM field of a downstream Dockerfile. "
        )
    elif image_type == "analytics":
        my_file.write("Optimized for Media Analytics. ")
        if image_name == "gst":
            my_file.write(
                "Included what are in media delivery GStreamer image, inferencing engine and video analytics plugins. "
            )
        if image_name == "ffmpeg":
            my_file.write(
                "Included what are in media delivery FFmpeg image, inferencing engine and video analytics plugins. "
            )
        ...
        if image_platform == "SG1" and image_name != "hddldaemon":
            my_file.write(
                "Also included Intel hardware accelaration software stack such as media SDK, media driver, opencl, gmmlib and libva. "
            )
        my_file.write(
            "The docker image can be used in the FROM field of a downstream Dockerfile. "
        )
        ...
        if image_name == "gst":
            my_file.write(
                "Included gstreamer and audio and video plugins that can be connected to process audio and video content, such as creating, converting, transcoding. "
            )
        if image_name == "ffmpeg":
            my_file.write(
                "Included FFmpeg and codecs such as opus, ogg, vorbis, x264, x265, VP8/9, SVT-AV1 and SVT-HEVC. "
            )
        if image_name == "ffmpeg-vmaf":
            my_file.write(
                "Included FFmpeg and codecs such as opus, ogg, vorbis, x264, x265, VP8/9, SVT-AV1 and SVT-HEVC. It also includes libvmaf for video quality assessment."
            )
        if image_name == "nginx":
            my_file.write(
                "Optimized for NGINX web server that can be used for serving web content, load balancing, HTTP caching, or a reverse proxy. "
            )
        if image_name == "svt":
            my_file.write(
                "Image with SVT (Scalable Video Technology) Encoder and decoders. Ready to use SVT apps to try AV1, HEVC, VP9 transcoders. "
            )
        if image_name == "srs":
            my_file.write(
                "Image with SRS high efficiency, stable and simple RTMP/HLS/FLV streaming cluster. "
            )
        if image_name == "imtl":
            my_file.write(
                "Image with IMTL for transmitting and receiving media data with high throughput and low latency. "
            )
        if image_platform == "SG1":
            my_file.write(
                "Also included Intel hardware accelaration software stack such as media SDK, media driver, gmmlib and libva. "
        ...
        my_file.write(
            "Optimized for video conferencing service based on the WebRTC technology and Open WebRTC Toolkit (OWT). "
        )
        if image_name == "owt":
            my_file.write(
                "Optimized for video conferencing service based on the WebRTC technology and Open WebRTC Toolkit (OWT). Included conferencing modes: 1:N, N:N with video and audio processing nodes. "
            )
        if image_name == "owt360":
            my_file.write(
                "Docker image optimized for ultra-high resolution immersive video low latency streaming, based on the WebRTC technology and the Open WebRTC Toolkit. Included SVT-HEVC tile-based 4K and 8K transcoding and field of view (FoV) adaptive streaming. "
            )
        my_file.write(
            "The docker image can be used in the FROM field of a downstream Dockerfile. "
        )
    ...
    my_file.write("\n\n")
    my_file.write(
        quick_reference(path, image_name, image_type, image_os, image_platform)
    )
    my_file.write(
        generate_license(path, image_name, image_type, image_os, image_platform)
    )
    my_file.close()
